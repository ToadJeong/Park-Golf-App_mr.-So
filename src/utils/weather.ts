...
export async function getWeatherDirect(latitude: number, longitude: number): Promise<WeatherData> {
  ...
  // In Vite, environment variables are prefixed with VITE_ and accessed via import.meta.env
  const kmaKey = import.meta.env.VITE_KMA_SERVICE_KEY;
  if (kmaKey) {
    try {
      ...
      // Use https for security context in mobile webviews
  ...
  // 4. Ultimate fallback based on current month
  if (temp === null) {
